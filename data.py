...
# def load_state_labels(userids, user_text_seq_full, user_loc, labels):
    ...
    # def load_state_labels(userids, user_text_seq_full, user_loc, labels):
        ...
        # y, X, userIDs = ([] for i in range(3))
        # for user in userids:
        #     if 'UKN' not in user_loc[user][0]:  # Use get() method to avoid KeyError
        #         if 'District of Columbia' in user_loc.get(user, {}).get('2', ''):  # Use get() method to avoid KeyError
        #             if user_loc.get(user, {}).get('0',
        #                                           '') == 'United States of America':  # Use get() method to avoid KeyError
        #                 X.append(user_text_seq_full[user])
        #                 userIDs.append(user)
        #                 if not user_loc.get(user, {}).get('2', '') in labels:
        #                     labels.append(user_loc.get(user, {}).get('2', ''))
        #                 y.append(labels.index(user_loc.get(user, {}).get('2', '')))
        #     if 'UKN' not in user_loc[user][1]:  # Use get() method to avoid KeyError
        #         if user_loc.get(user, {}).get('0',
        #                                       '') == 'United States of America':  # Use get() method to avoid KeyError
        #             X.append(user_text_seq_full[user])
        #             userIDs.append(user)
        #             if not user_loc.get(user, {}).get('1', '') in labels:
        #                 labels.append(user_loc.get(user, {}).get('1', ''))
        #             y.append(labels.index(user_loc.get(user, {}).get('1', '')))
        #
        #             if not userIDs:
        #                 return None
        #
...
#     for user in userids:
#         if 'UKN' not in user_loc[user][0]:  # Use get() method to avoid KeyError
#             if 'District of Columbia' in user_loc[user][2]:  # Use get() method to avoid KeyError
#                 if user_loc[user][0] == 'United States of America':  # Use get() method to avoid KeyError
#                     X.append(user_text_seq_full[user])
#                     userIDs.append(user)
#                     if not user_loc[user][2] in labels:
#                         labels.append(user_loc[user][2])
...
def load_state_labels(userids, user_text_seq_full, user_loc, labels):
    y, X, userIDs = ([] for i in range(3))
    for user in userids:
        if user in user_loc:  # Check if the user is in the user_loc dictionary
            user_info = user_loc[user]
            if 'UKN' not in user_info[0]:
                if 'District of Columbia' in user_info[2]:
                    if user_info[0] == 'United States of America':
                        X.append(user_text_seq_full[user])
                        userIDs.append(user)
                        if not user_info[2] in labels:
                            labels.append(user_info[2])
                        y.append(labels.index(user_info[2]))
            if 'UKN' not in user_info[1]:
                if user_info[0] == 'United States of America':
                    X.append(user_text_seq_full[user])
                    userIDs.append(user)
                    if not user_info[1] in labels:
                        labels.append(user_info[1])
                    y.append(labels.index(user_info[1]))
    return userIDs, y, labels
